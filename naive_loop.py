...
def predict(Theta1, Theta2, X):
    ...
    prediction = np.zeros((m, 1)) # output results
    
    # loop through the number of samples
    for t in range(m):
        a1 = X[t, :].reshape((input_layer_size, 1))
        a1 = np.vstack((1, a1)) # (input_layer_size+1, 1)

        z2 = Theta1 @ a1
        a2 = g(z2)
        a2 = np.vstack((1, a2)) # (hidden_layer_size+1, 1)

        a3 = g(Theta2 @ a2) # (num_labels, 1)

        prediction[t, 0] = np.argmax(a3)
...
def cost_function(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
    ...
    # *** naive loop-based implementation ***
    J = 0.0 # accumulator for the cost function
    ...
    for t in range(m):
        ...
        y_k = np.zeros((num_labels, 1)) # one-hot encoding for the target label
        y_k[int(y[t, 0])] = 1.0

        J_t = np.sum(-y_k * np.log(a3) - (1.0 - y_k) * np.log(1.0 - a3)) # Cross Entropy cost for individual sample t
        J += J_t

    J = J / m

    J += lmbda / (2.0 * m) * (np.sum(Theta1[:, 1:] ** 2) + np.sum(Theta2[:, 1:] ** 2)) # regularization
...
def main():
    ...
    # Save figure
    plt.savefig("artificialneuralnetwork_naive_loop.png", dpi=240)
    # plt.show()
...
if __name__ == "__main__":
    if os.environ.get("SKIP_PROFILING") == "1":
        main()
    else:
        os.environ["SKIP_PROFILING"] = "1"
        
        # *** (1) Measure total execution time ***
        print("\n" + "*"*60)
        print(" (1) Measure total execution time")
        print("*"*60)

        start_time = time.time()
        main()
        end_time = time.time()
        diff_baseline = end_time - start_time
        print(f"-> Baseline total execution time: {diff_baseline:.4f} seconds\n")

        # *** (2) Measure specific function execution time using cProfile ***
        print("\n" + "*"*60)
        print(" (2) Measure specific function execution time using cProfile")
        print("*"*60)

        start_time = time.time()
        cProfile.run('main()', 'profile_main.stats')
        end_time = time.time()
        
        p = pstats.Stats('profile_main.stats')
        p.sort_stats('cumulative').print_stats('predict|cost_function|gradient') # print prediction, cost function, and gradient functions
        diff_cprofile = end_time - start_time
        print(f"-> cProfile overhead included execution time: {diff_cprofile:.4f} seconds\n")

        # *** (3) Measure line-by-line execution time using line_profiler ***
        print("\n" + "*"*60)
        print(" (3) Measure line-by-line execution time using line_profiler")
        print("*"*60)

        lp = LineProfiler()

        lp.add_function(cost_function)
        lp.add_function(gradient)
        ...
        lp.print_stats()
        diff_lineprofiler = end_time - start_time
        print(f"-> line_profiler overhead included execution time: {diff_lineprofiler:.4f} seconds\n")

        # *** (4) Measure total program memory usage using mprof ***
        print("\n" + "*"*60)
        print(" (4) Measure total program memory usage using mprof")
        print("*"*60)

        script_name = sys.argv[0]
        
        subprocess.run(["mprof", "run", script_name], capture_output=True)
        subprocess.run(["mprof", "plot", "-o", "mprof_plot_naive_loop.png"], capture_output=True)
        print("mprof_plot_naive_loop.png saved")


"""
python naive_loop.py | tee output_log_naive_loop.txt
"""